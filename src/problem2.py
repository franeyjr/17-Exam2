...
class Triangle(object):
    ...
    def area(self):
        ...
        #        They include the Example in the above doc-string.
        # ---------------------------------------------------------------------
        s = (1/2) * (self.a.distance_from(self.b) + self.b.distance_from(self.c) + self.c.distance_from(self.a))
        answer = math.sqrt(s *(s- self.a.distance_from(self.b))*(s- self.b.distance_from(self.c))*(s- self.c.distance_from(self.a)))
        return answer.__round__()
    ...
    def shrink_or_expand(self, f):
        ...
        self.c.y = self.c.y * f
        new_area = self.area()
        if new_area > self.original_area:
            self.original_area = new_area
...
def run_test_return_doubled_triangle():
    ...
    expected_a = Point(100, 24)
    expected_b = Point(170, 80)
    expected_c = Point(40, 180)
    run_test_instance_variables(t2, expected_a, expected_b, expected_c)
